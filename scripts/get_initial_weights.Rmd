...
wsaSpat <- vect(wsAcec) 
wsAcec_areas <- terra::expanse(wsaSpat, unit = 'ha') * 2.47105
wsAcec_areas <- bind_cols(wsAcec, Total_Area =  wsAcec_areas)
...
wsAcec_areas <- left_join(wsAcec_areas, raster_cells) %>% 
  ...
  select(ID, NAME, TYPE, Total_Area, Code, PropArea, Area)

wsAcec_area_summary <- wsAcec_areas %>% 
  ungroup() %>% 
  group_by(Code) %>% 
  mutate(StratumArea = sum(Area)) 
  

rm(acecs, wsa, acec_pts, wsa_pts, raster_cells, wsaSpat, wsAcec)
```
