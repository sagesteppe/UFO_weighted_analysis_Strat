...
\begin{wrapfigure}{l}{0.4\textwidth}
  \centering
    \includegraphics[width=0.4\textwidth]{../graphics/maps/AIMPlots.png}
...
The Uncompahgre Field office has `r length(unique(acecs$NAME))` unique Areas of Critical Environmental Concern. The focus of these areas range from Archaeological, through sections of Rivers, to rare plant localities. In addition the field office has two additional Wilderness Study Areas, both associated with National Conservation Areas, but which are included in these analyses.
...
The Dominguez-Escalente National Conservation Area is joint administered with the Grand Junction Field Office, all values here refer only to the portions administered by the UFO field office. As mentioned previously, any lands within Wilderness Study Areas within the National Conservation Areas are included with the ACEC-WSA strata.
...
total_area <- round(as.numeric(measurements::conv_unit(st_area(DEnm), "m2", "acre")))
dearea <- TArea(DEnm)
...
total_area <- round(as.numeric(measurements::conv_unit(st_area(DEnm_only), "m2", "acre"))) 
...
\begin{wrapfigure}{l}{0.5\textwidth}
  \centering
    \includegraphics[width=0.5\textwidth]{../graphics/maps/DominguezEscalenteAIMPlots.png}
...
\end{wrapfigure}
 ...
 The total area of the Monument which is administered by the Uncompahgre Field Office is roughly `r dearea` acres. However due to the large WSA within it, and assorted ACEC's the total area is reduced to only `r de_only` acres...
...
\begin{wrapfigure}{l}{0.5\textwidth}
  \centering
    \includegraphics[width=0.5\textwidth]{../graphics/maps/GunnisonGorgeAIMPlots.png}
  \caption{All plots sampled in the vicinity of the National Conservation Area}
\end{wrapfigure}

The Gunnison Gorge National Conservation Area encompasses `r ggarea` acres, and contains only a few of the strata types covered in the design. It is the only sub-unit of the AIM sample design which is comprised primarily of salt desert, which is featured prominently along the Western portions of it. It also contains significant portion of Pinon-Juniper, adjacent to the cliffs overlooking the gorge, and some of the field offices best Sagebrush-Steppe along the Eastern portions. The area features only trace amount of `r NoStrata(GG_NM)` of the original strata, resulting in them being devoid of any plots in the original draw. There were `r MinStrata(GG_NM)` strata only present in minor amounts which had fewer than five plots drawn
...
Of the `r drawnStrata(GG_NM)` strata which had plots drawn within the NCA, only `r SampledStrata(GG_NM)` were successfully sampled. The Pinon-Juniper stratum, representing 29% of the area, only had one point sampled, accordingly no temporal analyses can be performed using this stratum. However, both the Salt Desert and Sagebrush-Steppe stratum, which compose roughly 55% of the NCA, had enough plots sampled for temporal analyses. ...
...
```{r}
rm(OriginalWeights_results, NewWeights_results, points, originalWeights, plotWeigher, GG_NM)
```
